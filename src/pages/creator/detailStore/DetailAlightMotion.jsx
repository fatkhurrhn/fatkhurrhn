    ...
    return (
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-[110px]">
        ...
        <div className="text-center flex-col sm:flex-row justify-between items-start sm:items-center mb-3 sm:mb-8 gap-4">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 pt-10">AM Pricelist</h1>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-2">
                            {item.options.map((option, idx) => (
                                <button
                                    ...
                                    className={`p-3 rounded-md border text-[14px] border-blue-300  text-black transition ${selected === `${item.category} - ${option.duration} : ${option.price}` ? 'bg-blue-900 text-white' : ''}`}
                                >
